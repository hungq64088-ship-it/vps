...
if ! [[ "$option" =~ ^[0-9]+$ ]]; then
    echo -e "${RED}Error: Please enter a valid number (1-4)${NC}"
    exit 1
fi
...
if [ "$option" -eq 1 ]; then
    ...
    read -p "$(echo -e ${YELLOW})Select RDP Port: $(echo -e ${NC})" selectedPort
    ...
    read -p "$(echo -e ${YELLOW})Enter PufferPanel Port: $(echo -e ${NC})" pufferPanelPort

    sed -i "s/\"host\": \"0.0.0.0:8080\"/\"host\": \"0.0.0.0:$pufferPanelPort\"/g" /etc/pufferpanel/config.json
    
    read -p "$(echo -e ${YELLOW})Enter the username for the admin user: $(echo -e ${NC})" adminUsername
    read -sp "$(echo -e ${YELLOW})Enter the password for the admin user: $(echo -e ${NC})" adminPassword
    echo ""
    read -p "$(echo -e ${YELLOW})Enter the email for the admin user: $(echo -e ${NC})" adminEmail
    ...
    echo -e "${GREEN}Installed: sudo / curl / wget / git / lsof / ping${NC}"
